...
public class ExportFinancialsQueryHandler : IRequestHandler<ExportFinancialsQuery, byte[]>
{
    ...
    public async Task<byte[]> Handle(ExportFinancialsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                throw new UnauthorizedAccessException("Hospital context is required to export financial data.");
            }
            ...
            if (!invoices.Any())
            {
                throw new InvalidOperationException("No invoices found for the specified date range.");
            }
        ...
        {
            throw new Exception($"Failed to export financial data: {ex.Message}", ex);
        }
    }
